...
namespace ops {
  ...
  using MatrixXdType = Eigen::Ref<const Eigen::MatrixXd>;
   ...
   /*
   * Abstract functor Initializer
   * Require implementation of the virtual call method
   * Should return an Eigen matrix
   */
  struct Initializer {
  public:
  ...
  **/
  ...
  public:
    UniformInitializer(const double lower, const double upper)
      : m_lower(lower), m_upper(upper), m_unifd(boost::random::uniform_real_distribution<>(lower, upper)) {}
    
    MatrixXdType operator()() override {
      return Eigen::MatrixXd::Zero(n_rows, n_cols).unaryExpr([&](double t){ return m_unifd(rng); });
    }
  ...
  public:
    NormalInitializer(const double mean, const double var)
      : m_mean(mean), m_var(var), m_gaussd(boost::normal_distribution<>(mean, var)) {}

    MatrixXdType operator()() override {
      boost::variate_generator<boost::mt19937&, boost::normal_distribution<>> gaussvars(rng, m_gaussd);
      return Eigen::MatrixXd::Zero(n_rows, n_cols).unaryExpr([&](double t){ return gaussvars(); });
    }
